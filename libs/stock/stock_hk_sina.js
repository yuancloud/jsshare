...
///新浪财经-港股的所有港股的实时行情数据
async function stock_hk_spot() {
    let hk_sina_stock_list_url = "http://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHKStockData"
    let hk_sina_stock_dict_payload = {
        "page": "1",
        "num": "3000",
        "sort": "symbol",
        "asc": "1",
     ...
     * @return {Array} 实时行情数据
     */
    const res = await axios.get(hk_sina_stock_list_url, { params: hk_sina_stock_dict_payload });
...
async function stock_hk_daily(symbol = "00981", adjust = "qfq") {
    ...
    if (adjust === '') {
        ...
        let response = await axios.get(`https://finance.sina.com.cn/stock/hkstock/${symbol}/hfq.js`);
        let str = response.data.split("=")[1].split("\n")[0];
        let records_hfq = JSON.parse(str);
        let result_hfq = records_hfq.data.map(e => {
            return {
                date: e.d,
                hfq_factor: e.f,
                cash: e.c
        ...
        let result = result_hfq.sort((a, b) => util.parseDate(b.date) - util.parseDate(a.date))
        for (let i = 0; i < result.length; i++) {
            let e = result[i]
            if (i === 0) {
                records.filter(f => f.date >= e.date).forEach(f => {
                    f.open = parseFloat((e.hfq_factor * f.open + e.cash).toFixed(4))
                    f.high = parseFloat((e.hfq_factor * f.high + e.cash).toFixed(4))
                    f.low = parseFloat((e.hfq_factor * f.low + e.cash).toFixed(4))
                    f.close = parseFloat((e.hfq_factor * f.close + e.cash).toFixed(4))
                })
            } else {
                let last = result[i - 1]
                records.filter(f => f.date >= e.date && f.date < last.date).forEach(f => {
                    f.open = parseFloat((e.hfq_factor * f.open + e.cash).toFixed(4))
                    f.high = parseFloat((e.hfq_factor * f.high + e.cash).toFixed(4))
                    f.low = parseFloat((e.hfq_factor * f.low + e.cash).toFixed(4))
                    f.close = parseFloat((e.hfq_factor * f.close + e.cash).toFixed(4))
        ...
        let response = await axios.get(`https://finance.sina.com.cn/stock/hkstock/${symbol}/qfq.js`);
        let str = response.data.split("=")[1].split("\n")[0];
        let records_qfq = JSON.parse(str);
        let result_qfq = records_qfq.data.map(e => {
            return {
                date: e.d,
                qfq_factor: e.f,
                cash: e.c
        ...
        let result = result_qfq.sort((a, b) => util.parseDate(b.date) - util.parseDate(a.date))
        for (let i = 0; i < result.length; i++) {
            let curr = result[i]
            if (i === 0) {
...
// 注意：上述代码中的 `decodeResponse` 函数和复权因子处理的具体实现需要根据实际情况编写。
module.exports = {
